...
---
### Description
A `zone` geography is mapped as a parent for some `region` geographies and then `deactivated` as in [TC-D-0002](obsidian://open?vault=software&file=Mobile%20App%201%2FReview%201%2FTest%20Cases%2FDashboard%2FGeography%2FDeactivating%20a%20Higher%20Level%20Geography) We should not see `Edit` action against `deactivated` zone. `Active` check-box should not be visible in `edit` form
<u>Pre-conditions</u>
...
| 4    | Check the Geography List action buttons against `deactivated zone`               | Should not show `Edit` button against `zone` - South                                                                  | `Deactivate & Edit` action buttons are showing                              | ==FAIL==                             |
