...
- PostgreSQL 16 Database running locally or in Docker on port `5432`
...
cp .env.example .env
# Ensure DB_PORT, DB_USER, DB_PASSWORD, DB_NAME match your PostgreSQL instance
...
- **`db`** — PostgreSQL 16 (Alpine) with persistent volume and health checks (`pg_isready`).
- **`app`** — Python container running FastAPI with Gunicorn + Uvicorn workers. Runs `alembic upgrade head` on start.
...
| `DB_HOST` | `127.0.0.1` | PostgreSQL Host IP |
| `DB_PORT` | `5432` | PostgreSQL Server Port |
| `DB_USER` | `safar_user` | PostgreSQL Database Username |
| `DB_PASSWORD` | `safar_password` | PostgreSQL Database Password |
| `DB_NAME` | `safar_db` | PostgreSQL Database Name |
